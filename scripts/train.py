...
from od.data.datasets import create_dataloader
...
def train(hyp, opt, device, callbacks):  # hyp is path/to/hyp.yaml or hyp dictionary
    ...
    if pretrained:
        ...
        model = Model(opt.cfg if opt.cfg != '' else ckpt['model'].yaml, nc=nc, anchors=ckpt['ema'].detection.anchors.reshape(-1, 6).numpy()).to(device)  # create
    ...
    # Model attributes
    nl = de_parallel(model).detection.nl  # number of detection layers (to scale hyps)
